...
class XenForoStatusBot:
    ...
    def get_daily_logins(self):
        query = "SELECT COUNT(*) FROM xf_user WHERE last_activity >= DATE_SUB(NOW(), INTERVAL 1 DAY)"
        self.cursor.execute(query)
        result = self.cursor.fetchone()
        return result[0] if result else 0

    def get_banned_users_count(self):
        query = "SELECT COUNT(*) FROM xf_user WHERE user_state = 'banned'"
        self.cursor.execute(query)
        result = self.cursor.fetchone()
        return result[0] if result else 0
...
if __name__ == "__main__":
    ...
    daily_logins = status_bot.get_daily_logins()
    banned_users_count = status_bot.get_banned_users_count()
    ...
    print(f"Günlük Giriş Yapan Kullanıcı Sayısı: {daily_logins}")
    print(f"Banlı Kullanıcı Sayısı: {banned_users_count}")
